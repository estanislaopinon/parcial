class Pila():
    ...
    def push(self, value):  # Agrega el elemento sobre la cima de la pila
        self.__elements.append(value)

    def pop(self):  # Elimina y devuelve el elemento almacenado en la cima de la pila
        if self.size() > 0:
            dato = self.__elements.pop()
            return dato

    def size(self):  # devuelve la cantidad de elementos en la pila
        return len(self.__elements)

    def on_top(self):  # devuelve el valor del elemento que esta almacenado en la cima de la pila pero sin eliminarlo
        if self.size() > 0:
